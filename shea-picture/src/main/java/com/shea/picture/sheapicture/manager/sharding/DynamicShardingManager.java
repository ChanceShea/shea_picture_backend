...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "shea_picture." + tableName) // 确保前缀合法
    ...
    public void createSpacePictureTable(Space space) {
        // 仅对旗舰版团队空间创建分表
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.ULTIMATE.getValue()) {
            ...
            try {
                ...
                log.error("创建空间图片分表失败，空间ID：{}", spaceId, e);
            }
        }
    }
}
